  <!-- Main Sidebar Container -->
  <aside class="main-sidebar sidebar-light-orange elevation-4">
      <!-- Brand Logo -->
      <a href="#" class="brand-link">
          <img src="../../../assets/images/logos/Logo-v8.jpg" alt="AdminLTE Logo" class="brand-image img-circle elevation-3" style="opacity: .8">
          <span class="brand-text font-weight-light">CONCRETOL</span>
      </a>

      <!-- Sidebar -->
      <div class="sidebar">
          <!-- Sidebar Menu -->
          <nav class="mt-2">
              <ul class="nav nav-pills nav-sidebar flex-column nav-flat" data-widget="treeview" role="menu" data-accordion="false">
                  <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->
                  <li class="nav-item ">
                      <a href="../index.php" class="nav-link">
                          <i class="nav-icon fas fa-tachometer-alt"></i>
                          <p>
                              Aseguramiento de calidad
                          </p>
                      </a>
                  </li>
                  <li class="nav-header">Muestras</li>
                  <li class="nav-item has-treeview">
                      <a href="create/index.php" class="nav-link ">
                          <p>
                              Crear Muestra

                          </p>
                      </a>
                  </li>
                  <li class="nav-item has-treeview">
                      <a href="expplantilla/" class="nav-link">
                          <p>
                              Exportar Plantilla

                          </p>
                      </a>
                  </li>
                  <li class="nav-item has-treeview">
                      <a href="exp_informe/" class="nav-link">
                          <p>
                              Exportar BD

                          </p>
                      </a>
                  </li>
                  <li class="nav-item has-treeview">
                      <a href="exp_consolidado/" class="nav-link">
                          <p>
                              Exportar Consolidado

                          </p>
                      </a>
                  </li>
                  <li class="nav-item has-treeview">
                      <a href="param_muestras/" class="nav-link">
                          <p>
                              Parametrizacion

                          </p>
                      </a>
                  </li>

              </ul>
          </nav>
          <!-- /.sidebar-menu -->
      </div>
      <!-- /.sidebar -->
  </aside>